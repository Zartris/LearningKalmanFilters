...
void Simulation::update()
{
    if (m_is_running && !m_is_paused)
    {
        ...
        for (unsigned i = 0; i < m_time_multiplier; ++i)
        {
            ...
            // IMU Measurement / Prediction Step
            if (m_sim_parameters.imu_enabled)
            {
                if (m_time_till_imu_measurement <= 0)
                {
                    ImuMeasurement meas = m_imu_sensor.generateImuMeasurement(m_car.getVehicleState().ax, m_car.getVehicleState().ay, m_car.getVehicleState().yaw_rate);
                    m_kalman_filter.predictionStep(meas, m_sim_parameters.time_step);
                    m_time_till_imu_measurement += 1.0 / m_sim_parameters.gyro_update_rate;
                }
...
void Simulation::render(Display &disp)
{
    ...
    for (const auto &meas : m_gps_measurement_history)
    {
