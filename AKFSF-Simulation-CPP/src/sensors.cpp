...
// IMU Sensor
ImuSensor::ImuSensor() : m_rand_gen(std::mt19937()), m_accel_noise_std(0.0), m_gyro_noise_std(0.0), m_gyro_bias(0.0), m_accel_bias(0.0) {}
void ImuSensor::reset() { m_rand_gen = std::mt19937(); }
void ImuSensor::setAccelNoiseStd(double std) { m_accel_noise_std = std; }
void ImuSensor::setGyroNoiseStd(double std) { m_gyro_noise_std = std; }
void ImuSensor::setImuGyroBias(double bias) { m_gyro_bias = bias; }
void ImuSensor::setImuAccelBias(double bias) { m_accel_bias = bias; }
ImuMeasurement ImuSensor::generateImuMeasurement(double sensor_accel_x, double sensor_accel_y, double sensor_yaw_rate)
{
    ImuMeasurement meas;
    ...
    meas.ax = sensor_accel_x + m_accel_bias + accel_dis(m_rand_gen);
    meas.ay = sensor_accel_y + m_accel_bias + accel_dis(m_rand_gen);
    meas.psi_dot = sensor_yaw_rate + m_gyro_bias + gyro_dis(m_rand_gen);
    return meas;
}
